...
class Multi_process_Qm9sDataset(Dataset):
    ...
    def get_unimol_data(self, atoms, smi, ir, raman, coordinates, dictionary, max_atoms=512, remove_Hs=False):
        ...
        return {'src_tokens': atom_vec, 'src_edge_type': edge_type, 'src_coord':coordinates, 'src_distance': dist, 'smi': smi, 'ir': ir, 'raman': raman}
    ...
    def __getitem__(self, idx):
        ...
        raman = data["raman"]
        raman = raman.unsqueeze(0).unsqueeze(1)
        atoms = data["atoms"]
        atoms = [periodictable.elements[int(z)].symbol for z in atoms]
        coordinates = data["coordinates"]
        data_graph = defaultdict(list)
        data_graph['unimol'].append(self.get_unimol_data(atoms, smi, ir, raman, coordinates, self.dictionary))
...
if __name__ == "__main__":
    import yaml
    with open("/home/zjh/project/mol_spectra/config/property_predict/random_split/multimodel_bace.yaml", "r") as f:
        config = yaml.safe_load(f)
    dataset = Multi_process_Qm9sDataset(config["val_db_path"],config["dict_path"],config)
    dataloader = DataLoader(dataset, batch_size=3, shuffle=True,collate_fn=Multi_process_batch_collate_fn)
    for i in range(2):
